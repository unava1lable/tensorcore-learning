...
bool run_case(const char *name, const GemmShape &shape, const std::vector<half> &A,
    ...
    std::vector<float> reference(c_elems, 0.0f);
    ...
    std::printf("[%s] max_abs_error=%.8f max_rel_error=%.8f normalized_error=%.8e ", name,
                stats.max_abs, stats.max_rel, stats.normalized);
    if (stats.first_mismatch >= 0) {
        std::printf("FAILED first_mismatch_index=%d reference=%.8f kernel=%.8f\n",
